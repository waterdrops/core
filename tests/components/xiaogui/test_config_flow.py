...
async def test_async_step_bluetooth_valid_device(hass: HomeAssistant) -> None:
    ...
    with patch("homeassistant.components.xiaogui.async_setup_entry", return_value=True):
...
async def test_async_step_bluetooth_not_enough_info_at_start(
    ...
    with patch(
        "homeassistant.components.xiaogui.config_flow.async_process_advertisements",
    ...
    assert result2["type"] == FlowResultType.CREATE_ENTRY
    assert result2["title"] == "Xiaogui Motion & Light"
    assert result2["data"] == {}
    assert result2["result"].unique_id == "aa:bb:cc:dd:ee:ff"


async def test_async_step_bluetooth_not_xiaogui(hass: HomeAssistant) -> None:
    """Test discovery via bluetooth not xiaogui."""
    with patch(
        "homeassistant.components.xiaogui.config_flow.async_process_advertisements",
...
async def test_async_step_user_with_found_devices(hass: HomeAssistant) -> None:
    ...
    with patch(
        "homeassistant.components.xiaogui.config_flow.async_discovered_service_info",
        return_value=[LIGHT_AND_SIGNAL_SERVICE_INFO],
    ):
        result = await hass.config_entries.flow.async_init(
            DOMAIN,
